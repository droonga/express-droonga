...
function Agent(options) {
  ...
  this._nodeName = this._hostName + '/' + NodeType.PROTOCOL_ADAPTER;
...
Agent.prototype.start = function() {
  this._logger.trace(LOG_PREFIX + 'Starting Serf agent (' + this._serf + ')');
  return Q.Promise((function(resolve, reject, notify) {
    ...
    if (this._serf.charAt(0) == '/' &&
      ...
      this._logger.info(LOG_PREFIX + 'Serf command is not found. Install Serf automatically.');
...
Agent.prototype.tryStart = function() {
  var agentArgs = [
    ...
    '-log-level', 'INFO',
    '-tag', Tag.nodeType + '=' + NodeType.PROTOCOL_ADAPTER
  ];
  ...
  try {
    this._logger.info(LOG_PREFIX + 'Starting Serf agent: ' + this._serf + ' ' + agentArgs.join(' '));
    this._agentProcess = spawn(this._serf, agentArgs);
    this._agentProcess.on('close', (function(exitStatusCode) {
      if (exitStatusCode != 0)
        this._logger.error(LOG_PREFIX, new Error('Serf agent is closed with error: ' + exitStatusCode));
      this._agentProcess = null;
    }).bind(this));
    ...
    this._logger.error(LOG_PREFIX + 'Failed to start Serf agent.',
                       error);
    try {
      this._agentProcess.kill();
    } catch(error) {
      this._logger.error(LOG_PREFIX + 'Failed to stop Serf agent.',
                         error);
    }
    this._agentProcess = null;
  }
...
Agent.prototype._handleOutput = function(output) {
  this._logger.trace(LOG_PREFIX + 'Serf agent output: ' + output);
...
Agent.prototype.shutdown = function() {
  this._logger.info(LOG_PREFIX + 'Shutting down Serf agent');
  return Q.Promise((function(resolve, reject, notify) {
