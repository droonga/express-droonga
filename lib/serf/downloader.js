...
var LOG_PREFIX = '[serf-downloader] ';
...
Downloader.prototype = {
  ...
  _trySave: function(url, dir, retryCount) {
    this._logger.debug(LOG_PREFIX + 'Serf Downloader: Trying to download ' + url +
        ...
        download.run((function(error, files, stream) {
          if (error) {
            if (retryCount > 0) {
              this._logger.error(LOG_PREFIX, error);
              setTimeout((function() {
                this._logger.error(LOG_PREFIX + 'Serf Downloader: Retrying to download.');
