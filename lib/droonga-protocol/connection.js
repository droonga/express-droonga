...
Connection.prototype._initReceiver = function() {
  ...
  logger.trace('Connection._initReceiver %d: %d %d:',
...
Connection.prototype._handleMessage = function(envelope) {
  ...
  if (inReplyTo) {
    ...
    logger.trace('Connection._handleMessage.reply %d:', this._id, inReplyTo);
    var errorCode = envelope.statusCode;
    if (!errorCode || isSuccess(errorCode))
      errorCode = null;
    this.emit('reply:' + inReplyTo, errorCode, envelope);
  } else {
    logger.trace('Connection._handleMessage.message %d:', this._id, envelope.type);
...
Connection.prototype.emitMessage = function(type, body, callback, options) {
  ...
  var id = createId();
  logger.trace('Connection.emitMessage %d:', this._id, id, type);
  ...
  if (callback) {
    ...
    this.once(event, function(errorCode, response) {
      logger.trace('Connection.emitMessage.reply %d:', this._id, errorCode);
