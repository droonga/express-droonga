        ...
        .then((function() {
          ...
          this._serf.on('member-change', (function() {
            ...
            if (this._updateHostNamesTimer) {
              this._logger.trace(LOG_PREFIX + 'Task to update host names is already running.');
              return;
            }
            this._updateHostNamesTimer = setTimeout((function() {
              this._updateHostNamesTimer = null;
              this._logger.trace(LOG_PREFIX + 'Starting to update host names...');
              this.updateHostNamesFromCluster()
                .then((function() {
                  if (this._hostNames.length == 0) {
                    this._logger.info(LOG_PREFIX + 'No service provider in the cluster. ' +
                                        'Trying to join to the initial cluster...');
                    return this.reJoinToInitialCluster();
                  }
                }).bind(this))
                .catch((function(error) {
                  this._logger.error(LOG_PREFIX + 'Failed to update host names.',
                                     error);
                  reject(error);
                }).bind(this));
            }).bind(this), 500);
  ...
  endWatchClusterChanges: function() {
    return Q.Promise((function(resolve, reject, notify) {
      ...
      this._serf.shutdown()
                  ...
                  .catch((function(error) {
                    this._logger.error(LOG_PREFIX + 'Failed to stop watching.',
                                       error);
                    reject(error);
                  }).bind(this));
    }).bind(this));
